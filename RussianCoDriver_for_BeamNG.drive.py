...
# Текущая версия программы (меняй при каждом обновлении)
CURRENT_VERSION = "1.0.0"

# Ссылка на raw-версию version.json в твоём репозитории
VERSION_URL = "https://raw.githubusercontent.com/PetrosyanDobryakov/BeamNG.drive_Rally-RussianCoDriver/main/version.json"

# Ссылка на страницу загрузки последнего релиза (заменишь на свою, если понадобится)
DOWNLOAD_URL = "https://github.com/PetrosyanDobryakov/BeamNG.drive_Rally-RussianCoDriver/releases/latest"

# Ссылка для сообщений об ошибках
ISSUES_URL = "https://github.com/PetrosyanDobryakov/BeamNG.drive_Rally-RussianCoDriver/issues/new"
...
# Инициализация виджетов, которые создадутся в main
debug_text = None
start_btn = None
stop_btn = None
...
def load_mappings():
    ...
    try:
        ...
        update_audio_paths_in_mapping(component_map)
    ...
    if os.path.exists(SYSTEM_MAPPING_FILE):
        ...
        update_audio_paths_in_mapping(system_map)
...
def check_for_update():
    """Проверяет наличие новой версии через version.json на GitHub."""
    try:
        # Пробуем обычный запрос с таймаутом 10 секунд
        with urllib.request.urlopen(VERSION_URL, timeout=10) as resp:
            data = json.load(resp)
    except Exception:
        try:
            # Запасной вариант: отключаем проверку SSL (нужно только для GitHub)
            ctx = urllib.request.SSLContext()
            # Создаём небезопасный контекст (игнорируем ошибки сертификата)
            ctx.check_hostname = False
            ctx.verify_mode = ssl.CERT_NONE
            with urllib.request.urlopen(VERSION_URL, timeout=10, context=ctx) as resp:
                data = json.load(resp)
        except Exception as e:
    ...
    if latest > CURRENT_VERSION:
        if messagebox.askyesno("Доступна новая версия",
                               f"Версия {latest} доступна для скачивания.\nПерейти на страницу загрузки?"):
            webbrowser.open(DOWNLOAD_URL)
    else:
        messagebox.showinfo("Обновлений нет", "У вас актуальная версия.")
...
def open_settings():
    ...
    btn_version = tk.Label(link_frame, text="Проверить версию", fg="blue", cursor="hand2", font=("Arial", 10, "underline"))
    btn_version.pack(side=tk.LEFT, padx=5)
    btn_version.bind("<Button-1>", lambda e: check_for_update())

    # Кнопка «Сообщить об ошибке»
    btn_report = tk.Label(link_frame, text="Сообщить об ошибке", fg="blue", cursor="hand2", font=("Arial", 10, "underline"))
    btn_report.pack(side=tk.LEFT, padx=5)
    btn_report.bind("<Button-1>", lambda e: webbrowser.open(ISSUES_URL))
